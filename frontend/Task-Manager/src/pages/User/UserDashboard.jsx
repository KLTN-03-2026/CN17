...
const MemberDashboard = () => {
    ...
    const prepareChartData = (charts) => {
        ...
        setPieChartData([
            {
                status: "Chờ xử lý",
                count: dist?.pending || dist?.Pending || 0,
            },
            {
                status: "Đang làm",
                count:
            ...
            {
                status: "Hoàn thành",
                count: dist?.completed || dist?.Completed || 0,
            },
        ]);
    ...
    const fetchProjectNotifications = async (projectId) => {
        try {
            const res = await axiosInstance.get(
                API_PATHS.NOTIFICATIONS.GET_ACTIVE_BY_PROJECT(projectId)
            );

            setProjectNotifications(res.data.notifications || []);
            setShowNotifications(true);
        } catch (error) {
            console.error("Lỗi tải thông báo dự án:", error);
    ...
    const fetchPendingInvites = async () => {
        try {
            const res = await axiosInstance.get(
                API_PATHS.INVITATIONS.GET_MY_INVITATIONS
            );
    ...
    useEffect(() => {
        if (selectedProject) {
            fetchDashboardData(selectedProject._id);
            fetchProjectNotifications(selectedProject._id);
        }
    }, [selectedProject]);
                    ...
                    projectNotifications.length > 0 && (
                        <div className="mt-5 space-y-3">
                            {projectNotifications.map((item) => (
                                <div
                                    key={item._id}
                                    className="relative border border-amber-200 bg-amber-50 rounded-xl p-4"
                                >
                                    <button

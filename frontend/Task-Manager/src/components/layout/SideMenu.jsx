    ...
    return (
        <div className="w-52 h-[calc(100vh-61px)] bg-white border-r border-gray-200/50 sticky top-[61px] z-20">
            <div className="flex flex-col items-center justify-center mb-4 pt-5">
                ...
                <div className={`text-[10px] font-medium text-white ${badge.className} px-3 py-0.5 rounded mt-1`}>
                    {badge.label}
                </div>
            ...
            {sideMenuData.map((item, index) => (
                <button
                    key={`menu_${index}`}
                    className={`w-full flex items-center gap-3 text-[13px] ${
                        activeMenu === item.label
                            ? "text-primary bg-linear-to-r from-blue-50/40 to-blue-100/50 border-r-3"
                            : ""
                    } py-2 px-6 mb-1 cursor-pointer`}

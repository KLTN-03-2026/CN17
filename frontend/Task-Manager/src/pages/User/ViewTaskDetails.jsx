...
import { LuSquareArrowOutUpRight, LuPlus, LuTrash2, LuCircleDot, LuCircle } from 'react-icons/lu';
...
const ViewTaskDetails = () => {
  ...
  const getStatusTagColor = (status) => {
    switch (status) {
      ...
      case "overdue":     return "text-red-500 bg-red-50 border border-red-200";
  ...
  const completedSub = task?.todoChecklist?.filter((i) => i.completed).length || 0;
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 mt-4 gap-4">
            <div className="form-card col-span-3">
              ...
              <div className="grid grid-cols-12 gap-4 mt-4">
                <div className="col-span-6 md:col-span-4">
                  <InfoBox 
                    ...
                    value={task?.priority === "high" ? "Cao" : task?.priority === "medium" ? "Trung bình" : "Thấp"} 
                  />
                </div>
                <div className="col-span-6 md:col-span-4">
                  <InfoBox
                    label="Hạn chót"
                    value={task?.dueDate ? moment(task?.dueDate).format("DD [Tháng] MM, YYYY") : "Chưa thiết lập"}
                  />
                </div>
                <div className="col-span-6 md:col-span-4">
                  <label className="text-xs font-medium text-slate-500">Người thực hiện</label>
                  <AvatarGroup
              ...
              <div className="mt-6">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <label className="text-xs font-medium text-slate-500">Công việc con</label>
                    {totalSub > 0 && (
                      <span className="text-xs text-gray-400">{completedSub}/{totalSub} hoàn thành</span>
                    )}
                  </div>
                  {task?.status !== "overdue" && (
                    <button
                    ...
                    >
                      <LuPlus className="text-sm" /> Thêm việc con
                    </button>
                  )}
                </div>
                ...
                {totalSub > 0 && (
                  <div className="mb-3">
                    ...
                    <p className="text-[10px] text-gray-400 mt-1 text-right">Tiến độ: {progress}%</p>
                  </div>
                )}
                ...
                {task?.todoChecklist?.map((item, index) => (
                  ...
                  >
                    <button onClick={() => updateTodoChecklist(index)} className="shrink-0 text-lg">
                      {item.completed
                    ...
                    <div className="flex-1 flex items-center gap-2">
                      ...
                      <p className={`text-[13px] ${item.completed ? "line-through text-slate-400" : "text-gray-800"}`}>
                        {item.text}
                      </p>
                    </div>
                    {task?.status !== "overdue" && (
              ...
              {task?.attachments?.length > 0 && (
                <div className="mt-6">
                  <label className="text-xs font-medium text-slate-500">Tài liệu đính kèm</label>
...
const InfoBox = ({ label, value }) => (
  <div className="mb-2">
    ...
    <p className="text-[12px] md:text-[13px] font-medium text-gray-700 mt-0.5">{value || "Trống"}</p>
  </div>
);
